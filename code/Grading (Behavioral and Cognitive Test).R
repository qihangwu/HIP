...
data_BT = data %>%
  ...
  mutate(across(BT[c(1:18)], ~recode(.,
                                     `0` = 'A',
                                     `1` = 'B',
                                     `-9` = 'Missing'))) 
...
# Question regarding values to put for 1-6
...
data_DX = data %>%
  ...
  mutate(card_qual= recode(card_qual,
                                     `0` = 'Two plies have the same number of cards.',
                                     `1` = 'Two plies have similar number of cards but not exactly the same.',
